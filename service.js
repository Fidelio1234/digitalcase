    ...
    try {
      ...
      if (payload.tipo === 'rt') {
        ...
        if (marca === 'rch') {
          ...
          // tipo 1 = estesa (1F), tipo 2 = breve (7F). Default estesa se non specificato
          // (comportamento storico, da confermare con tecnico 3i quale sia il default corretto).
          let codiceChiusura = '1F'
          if (azione === 'chiusura_fiscale' && dati?.tipo === 2) codiceChiusura = '7F'
          if (azione === 'chiusura_fiscale') {
            const labelTipo = codiceChiusura === '7F' ? 'BREVE' : 'ESTESA'
            console.log(`🧾 Chiusura fiscale 3i richiesta: tipo=${dati?.tipo ?? '(non specificato, default estesa)'} → codice ${codiceChiusura} (${labelTipo})`)
          }
