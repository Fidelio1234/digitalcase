...
// Token per autorizzare lo shutdown remoto.
// Se non impostato in .env.local, l'endpoint /shutdown resta disabilitato (fail-safe).
const SHUTDOWN_TOKEN = env.SHUTDOWN_TOKEN || null

// ─── Whitelist Origin ──────────────────────────────────────────────────────
// Accetta solo richieste browser provenienti da *.digitalcase.it (produzione)
// o da localhost (sviluppo locale). Le richieste senza header Origin
// (es. service-polling.js → service.js, sulla stessa macchina) sono sempre permesse.
const ORIGIN_REGEX = /^https?:\/\/([a-z0-9-]+\.)?digitalcase\.it$|^https?:\/\/localhost(:\d+)?$/i

function originConsentito(origin) {
  if (!origin) return true // chiamata server-to-server, niente Origin da validare
...
const server = http.createServer(async (req, res) => {
  ...
  // Echo dell'Origin solo se è nella whitelist (mai '*')
  ...
  if (req.url === '/shutdown') {
    // ── Protezione: serve un token configurato in .env.local ────────────────
    if (!SHUTDOWN_TOKEN) {
...
async function inviaTCPRT(ip, porta, comando, marca) {
  return new Promise((resolve, reject) => {
    ...
    let risposta = ''
    const t0 = Date.now()
    console.log(`[RT] →  connetto a ${ip}:${porta} | comando: ${comando.slice(0, 60)}...`)
    client.setTimeout(8000)
    client.connect(parseInt(porta), ip, () => {
      console.log(`[RT] ✓ connesso dopo ${Date.now() - t0}ms`)
      const terminatore = marca === '3i' ? '\r' : '\r\n'
      client.write(comando + terminatore)
    })
    client.on('data', d => { risposta += d.toString(); console.log(`[RT] ← dati ricevuti dopo ${Date.now() - t0}ms:`, d.toString()) })
    client.on('end', () => { console.log(`[RT] ← end dopo ${Date.now() - t0}ms`); client.destroy(); resolve(risposta || 'OK') })
    client.on('close', () => { console.log(`[RT] ← close dopo ${Date.now() - t0}ms`); client.destroy(); resolve(risposta || 'OK') })
    client.on('timeout', () => { console.log(`[RT] ⏱ TIMEOUT dopo ${Date.now() - t0}ms`); client.destroy(); reject(new Error('Timeout cassa')) })
    client.on('error', err => {
      console.log(`[RT] ✗ errore dopo ${Date.now() - t0}ms:`, err.code, err.message)
      client.destroy()
      if (err.code === 'ECONNRESET' || err.code === 'EPIPE') resolve(risposta || 'OK')
      else reject(err)
    })
    ...
    try {
      ...
      if (payload.tipo === 'rt') {
        ...
        if (marca === 'rch') {
          ...
          // tipo 1 = chiusura fiscale semplice (1F), tipo 2 = chiusura con report reparti+IVA allegati (7F).
          // Confermato dal manuale 3i ufficiale (par. 3.5/9): NON esiste "breve/estesa", sono entrambe
          // chiusure fiscali reali — 7F include in più i rapporti reparti e IVA in azzeramento.
          let codiceChiusura = '1F'
          if (azione === 'chiusura_fiscale' && dati?.tipo === 2) codiceChiusura = '7F'
          if (azione === 'chiusura_fiscale') {
            const labelTipo = codiceChiusura === '7F' ? 'CON REPORT REPARTI+IVA' : 'SEMPLICE'
            console.log(`🧾 Chiusura fiscale 3i richiesta: tipo=${dati?.tipo ?? '(non specificato, default semplice)'} → codice ${codiceChiusura} (${labelTipo})`)
          }
          const cmds = azione === 'chiusura_fiscale' ? ['z', codiceChiusura, 'c'] : ['x', '1F', 'c']
          const risposta = await inviaTCP3i(ip, porta || 1723, cmds)
          console.log(`🧾 Chiusura fiscale 3i — comandi inviati: [${cmds.join(', ')}] — risposta cassa: ${JSON.stringify(risposta)}`)
          risultato = { ok: true, risposta }
        } else {
          let comando = dati?.cmd || ''
          if (!comando && azione) {
