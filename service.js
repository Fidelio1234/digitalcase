...
// Legge IP cassa da Supabase al primo ping RT ricevuto
let keepAliveIp = null
let keepAlivePorta = null
let keepAliveInterval = null

function avviaKeepAlive(ip, porta) {
  if (keepAliveInterval) return // già avviato
  keepAliveIp = ip
  keepAlivePorta = porta
  keepAliveInterval = setInterval(async () => {
    try {
      await inviaTCPRT(keepAliveIp, keepAlivePorta, 'K', '3i')
        ...
        if (marca === 'rch') {
          ...
          if (!comando && azione) {
            switch (azione) {
              ...
              case 'annullo': {
                if (marca === '3i') {
                  // Flusso 3i: prima 50F (verifica), poi 52F (annulla)
              ...
              case 'ristampa': {
                if (marca === '3i') {
                  ...
                  comando = String.raw`"${d1}${n1}${n2}"15F`
                } else {
...
async function inviaTCPDitron(cassaIp, cassaPorta, righe) {
  return new Promise((resolve, reject) => {
    const client = new net.Socket()
    let risposta = ''
    client.setTimeout(10000)
    client.connect(parseInt(cassaPorta), cassaIp, async () => {
...
const server = http.createServer(async (req, res) => {
  ...
  req.on('end', async () => {
    try {
      ...
      if (payload.tipo === 'rt') {
        const { marca, ip: cassaIp, porta: cassaPorta, azione, dati, comandi } = payload

        if (!cassaIp) throw new Error('IP mancante')
        if (marca === '3i') avviaKeepAlive(cassaIp, cassaPorta || 1723)

        let risultato

        if (marca === 'rch') {
          risultato = await inviaRCH(cassaIp, cassaPorta || 80, comandi || [])
        } else if (marca === '3i' && (azione === 'chiusura_fiscale' || azione === 'lettura_x')) {
          const cmds = azione === 'chiusura_fiscale' ? ['z', '1F', 'c'] : ['x', '1F', 'c']
          const risposta = await inviaTCP3i(cassaIp, cassaPorta || 1723, cmds)
          risultato = { ok: true, risposta }
        } else {
          let comando = dati?.cmd || ''
          if (!comando && azione) {
            switch (azione) {
              ...
              case 'annullo': {
                if (marca === '3i') {
                  ...
                  const ndoc = String(dati.numeroDocumento).padStart(4, '0')
                  const ref = `"${nazz}-${ndoc}"`
                  const verifica = await inviaTCPRT(cassaIp, cassaPorta || 1723, `${ref}50F`, '3i')
                  const esito = verifica.trim()[0]
                  if (esito !== '0') {
                    const errori = {'1':'Scontrino non trovato','2':'Non è documento di vendita','3':'Già annullato/reso','4':'Aliquote IVA diverse','5':'Aliquota non associata a reparto'}
                    throw new Error(errori[esito] || `Errore 3i: ${verifica}`)
                  }
                  const risposta = await inviaTCPRT(cassaIp, cassaPorta || 1723, `${ref}52F`, '3i')
                  res.writeHead(200, { 'Content-Type': 'application/json' })
                  res.end(JSON.stringify({ ok: true, risposta }))
                  return
                } else {
              ...
              case 'scontrino': {
                ...
                lines.push(dati.metodo === 'contanti' ? `chius t=1, imp=${tot}` : `chius t=5, imp=${tot}`)
                if (marca === 'ditron') {
                  const risposta = await inviaTCPDitron(cassaIp, cassaPorta || 1471, lines)
                  res.writeHead(200, { 'Content-Type': 'application/json' })
                  res.end(JSON.stringify({ ok: true, risposta }))
                  return
                }
          ...
          if (!comando) throw new Error('Comando RT mancante')
          const risposta = await inviaTCPRT(cassaIp, cassaPorta || 1471, comando, marca)
          risultato = { ok: true, risposta }
        }
      ...
      // ── Stampante Comande ────────────────────────────────────────────────
      const { ip: stampIp, porta: stampPorta, tavolo, righe, tipo, uscita, totaleUscite } = payload
      const doc = buildComanda(tavolo, righe, tipo || 'comanda', uscita, totaleUscite)
      await inviaTCP(stampIp, stampPorta || 9100, doc)
      res.writeHead(200, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({ ok: true }))
      console.log(`✓ Stampato: Tavolo ${tavolo} - ${righe.length} righe - tipo: ${tipo}`)
