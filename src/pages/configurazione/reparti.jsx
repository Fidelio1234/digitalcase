...
import { getReparti, saveReparti, generateId, formatEuro, parseEuro } from '@/lib/storage'
...
export default function RepartiPage() {
  ...
  function saveReparto() {
    ...
    let updated
    if (modal.mode === 'add') {
      updated = [...reparti, { ...form, id:generateId() }]
    } else {
      updated = reparti.map(r => r.id === modal.id ? { ...r, ...form } : r)
    }
    setReparti(updated); saveReparti(updated)
  ...
  function moveReparto(id, dir) {
    ...
    setReparti(newArr); saveReparti(newArr)
  }
