...
// Helper: chiama il registratore via service locale o remoto (usa serviceIp da rtConfig)
...
export default function PannelloRT({ rtConfig, mappatura, scontrino, onClose, onStampa }) {
  ...
  async function chiamaRT(azione, dati) {
    ...
    try {
      ...
      if (rtConfig.marca === 'rch') {
        ...
        data = await callRT('rch', { ip: rtConfig.ip, porta: rtConfig.porta || 80, comandi }, rtConfig.serviceIp)
      } else {
        let cmd = null
        if (azione === 'annullo') {
          if (rtConfig.marca === '3i') {
          ...
          if (rtConfig.marca === '3i') {
            ...
            const data3i = fmtData3i(dati.dataInizio)
            const n1 = String(dati.dalNumero || 0).padStart(5, '0')
            const n2 = String(dati.alNumero || 65535).padStart(5, '0')
            cmd = `"${data3i}${n1}${n2}"15F`
          } else {
            cmd = `dgfe tipo=160, datada=${dati.dataInizio}, dataa=${dati.dataFine}, nda=${dati.dalNumero}, na=${dati.alNumero}`
          }
        }
  ...
  async function salvaChiusura(stato) {
    ...
    const { data: ultimaChiusura } = await supabase
      ...
      .limit(1).single()
    const dataFrom = ultimaChiusura?.timestamp_chiusura || (oggi + 'T00:00:00')
    const { data: scontriniDb } = await supabase
      .from('scontrini').select('totale, metodo')
      .eq('negozio_id', NEGOZIO_ID)
      .gt('timestamp_emissione', dataFrom)
      .lte('timestamp_emissione', ora)
    const totaleGiornaliero = scontriniDb?.reduce((a, s) => a + s.totale, 0) || 0
    const totaleCarte = scontriniDb?.filter(s => s.metodo === 'carta').reduce((a, s) => a + s.totale, 0) || 0
    const totaleContanti = scontriniDb?.filter(s => s.metodo === 'contanti').reduce((a, s) => a + s.totale, 0) || 0
    const numeroScontrini = scontriniDb?.length || 0
    const { data: cont } = await supabase
      .from('contatori').select('chiusure')
      .eq('negozio_id', NEGOZIO_ID).eq('data', oggi).single()
    const numChiusura = (cont?.chiusure || 0) + 1
  ...
  async function letturaX(tipo) { await chiamaRT('lettura_x', { tipo }) }

  function annulloScontrino() {
    if (!annulloData.nazz || !annulloData.ndoc || !annulloData.data) {
      setConferma({ titolo: 'Dati mancanti', messaggio: 'Compila N° Chiusura, N° Documento e Data prima di procedere.', colore: '#ffb830', onConferma: null })
      return
    }
    setConferma({
      titolo: 'Annullo Scontrino RT',
      messaggio: `Confermi annullo scontrino n° ${annulloData.ndoc} chiusura ${annulloData.nazz}? Operazione comunicata AdE.`,
      colore: '#ff4d6a',
      onConferma: () => chiamaRT('annullo', { matricola: rtConfig.matricola, numeroAzzeramento: annulloData.nazz, numeroDocumento: annulloData.ndoc, data: annulloData.data })
    })
  }

  async function ristampaDGFE() { await chiamaRT('ristampa', ristampaData) }

  const btnStyle = (color) => {
    const c = color || '#252830'
    return { padding:'10px 16px', borderRadius:10, border:'1px solid ' + c + '44', cursor: loading ? 'not-allowed' : 'pointer', background: c === '#252830' ? '#1a1c24' : c + '22', color: c === '#252830' ? '#eef0f6' : c, fontSize:'0.82rem', fontFamily:"'DM Mono',monospace", opacity: loading ? 0.6 : 1, display:'flex', alignItems:'center', gap:8 }
  }

  const inputStyle = { width:'100%', boxSizing:'border-box', background:'#1a1c24', border:'1px solid #252830', borderRadius:6, padding:'4px 8px', color:'#00ff00', fontSize:'1.1rem', textAlign:'center', fontFamily:"'DM Mono',monospace" }

  return (
    <div style={{ position:'fixed', top:8, left:220, zIndex:400 }}>
      <div onClick={() => setPannelloAperto(!pannelloAperto)} style={{ padding:'8px 14px', cursor:'pointer', display:'flex', alignItems:'center', gap:8, background:'#1a1c24', borderRadius:10, border:'1px solid #252830' }}>
        <div style={{ width:8, height:8, borderRadius:'50%', background: rtConfig.attivo ? '#00e5a0' : '#5a5d6e', boxShadow: rtConfig.attivo ? '0 0 8px #00e5a0' : 'none' }} />
        <span style={{fontSize:'0.78rem', fontFamily:"'DM Mono',monospace", color:'yellow'}}>MENU RT {rtConfig.marca?.toUpperCase()}</span>
        <span style={{color:'#5a5d6e', fontSize:'0.75rem'}}>{pannelloAperto ? '▼' : '▲'}</span>
      </div>

      {pannelloAperto && (
        <div style={{ position:'fixed', inset:0, zIndex:500, background:'rgba(8,9,12,0.85)', display:'flex', alignItems:'center', justifyContent:'center' }} onClick={() => setPannelloAperto(false)}>
          <div style={{ background:'#111318', border:'1px solid #252830', borderRadius:20, padding:24, width:520, maxWidth:'95vw', maxHeight:'90vh', overflowY:'auto', boxShadow:'0 24px 64px rgba(0,0,0,0.7)', display:'flex', flexDirection:'column', gap:14 }} onClick={e => e.stopPropagation()}>

            <div style={{display:'flex', alignItems:'center', justifyContent:'space-between'}}>
              <div style={{color:'yellow', fontSize:'2rem'}}>MENU RT - {rtConfig.marca?.toUpperCase()}</div>
              <button onClick={() => setPannelloAperto(false)} style={{background:'transparent', border:'none', color:'#5a5d6e', cursor:'pointer', fontSize:'1.2rem'}}>✕</button>
            </div>
            ...
            <div>
              ...
              <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6, marginBottom:6}}>
                <div>
                  <label style={{fontSize:'12px', color:'#ff4d6a'}}>N° CHIUSURA</label>
                  <input type="number" value={annulloData.nazz} onChange={e => setAnnulloData(d => ({...d, nazz: e.target.value}))} placeholder="0001" style={inputStyle} />
                </div>
                <div>
                  <label style={{fontSize:'12px', color:'#ff4d6a'}}>N° DOCUMENTO</label>
                  <input type="number" value={annulloData.ndoc} onChange={e => setAnnulloData(d => ({...d, ndoc: e.target.value}))} placeholder="0001" style={inputStyle} />
                </div>
                <div>
                  <label style={{fontSize:'12px', color:'#ff4d6a'}}>DATA (DDMMYY)</label>
                  <input type="text" value={annulloData.data} onChange={e => setAnnulloData(d => ({...d, data: e.target.value}))} placeholder="010126" maxLength={6} style={inputStyle} />
                </div>
              </div>
              <button style={btnStyle('#ff4d6a')} onClick={annulloScontrino} disabled={loading}>Annulla scontrino RT</button>
            </div>
            ...
            <div>
              ...
              <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:6, marginBottom:6}}>
                <div>
                  <label style={{fontSize:'12px', color:'#00ff00'}}>DA DATA (DDMMYY)</label>
                  <input type="text" value={ristampaData.dataInizio} onChange={e => setRistampaData(d => ({...d, dataInizio: e.target.value}))} maxLength={6} style={inputStyle} />
                </div>
                <div>
                  <label style={{fontSize:'12px', color:'#00ff00'}}>A DATA (DDMMYY)</label>
                  <input type="text" value={ristampaData.dataFine} onChange={e => setRistampaData(d => ({...d, dataFine: e.target.value}))} maxLength={6} style={inputStyle} />
                </div>
                <div>
                  <label style={{fontSize:'12px', color:'#00ff00'}}>DA N° SCONTRINO</label>
                  <input type="number" value={ristampaData.dalNumero} onChange={e => setRistampaData(d => ({...d, dalNumero: parseInt(e.target.value)}))} placeholder="0 = tutti" style={inputStyle} />
                </div>
                <div>
                  <label style={{fontSize:'12px', color:'#00ff00'}}>A N° SCONTRINO</label>
                  <input type="number" value={ristampaData.alNumero} onChange={e => setRistampaData(d => ({...d, alNumero: parseInt(e.target.value)}))} placeholder="0 = tutti" style={inputStyle} />
                </div>
              </div>
              <button style={btnStyle('#00ff00')} onClick={ristampaDGFE} disabled={loading}>Ristampa scontrini</button>
            </div>

            {loading && <div style={{textAlign:'center', color:'#5a5d6e', fontSize:'0.8rem'}}>⏳ Comunicazione con la cassa...</div>}
            {risultato && (
              <div style={{ padding:'10px 12px', borderRadius:8, background: risultato.ok ? 'rgba(0,229,160,0.1)' : 'rgba(255,77,106,0.1)', border: '1px solid ' + (risultato.ok ? '#00e5a040' : '#ff4d6a40'), fontSize:'0.75rem', fontFamily:"'DM Mono',monospace", color: risultato.ok ? '#00e5a0' : '#ff4d6a' }}>
                {risultato.ok ? '✓ ' : '✗ '}{risultato.risposta || risultato.error || 'OK'}
              </div>
            )}
          </div>
      ...
      {conferma && (
        <div style={{ position:'fixed', inset:0, zIndex:600, background:'rgba(8,9,12,0.92)', display:'flex', alignItems:'center', justifyContent:'center' }}>
          <div style={{ background:'#111318', border:'1px solid ' + conferma.colore + '44', borderRadius:20, padding:28, width:380, maxWidth:'90vw', display:'flex', flexDirection:'column', gap:16 }}>
            <div style={{fontSize:'1rem', fontWeight:700, color:'#eef0f6'}}>{conferma.titolo}</div>
            <div style={{fontSize:'0.82rem', color:'#aeb0bc', lineHeight:1.6}}>{conferma.messaggio}</div>
            <div style={{display:'flex', gap:10}}>
              <button onClick={() => setConferma(null)} style={{ flex:1, padding:'10px', borderRadius:10, border:'1px solid #252830', background:'transparent', color:'#eef0f6', cursor:'pointer', fontSize:'0.85rem' }}>Annulla</button>
              {conferma.onConferma && (
                <button onClick={() => { setConferma(null); conferma.onConferma() }} style={{ flex:1, padding:'10px', borderRadius:10, border:'none', background:conferma.colore, color:'#08090c', cursor:'pointer', fontSize:'0.85rem', fontWeight:700 }}>Conferma</button>
              )}
            </div>
          </div>
        </div>
